...
#         self.output = nn.Linear(64, len(mapping))  # Output layer size based on your mapping
...
# def hybrid_predict(model, audio_path, device, threshold=0.65):
...
#         predicted_index = np.argmax(probabilities)  # Adjust index to match mapping
#         confidence = probabilities[predicted_index]
    
#     predicted_label = mapping[predicted_index]
#     result = "Real Voice" if predicted_label == "gt" else "AI Generated"
    
#     print(f"Model Prediction: {predicted_label} ({result}) with confidence: {confidence:.4f}")
...
def hybrid_predict(model, audio_path, device, threshold=0.65):
    ...
    try:
        ...
        # Adjust confidence based on pitch
        adjusted_confidence = confidence * 0.7 if pitch < 80 else confidence
...
if __name__ == "__main__":
    MODEL_PATH = r"C:\\Users\\GOOD\\Desktop\\Komil\\audio_preprocessing\\audio_classification_model.pth"
    AUDIO_PATH = r"C:\\Users\\GOOD\\Desktop\\Komil\\audio_preprocessing\\14_Solving_overfitting_in_neural_network\\code\\gt.wav"
